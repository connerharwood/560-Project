...
waterUse_raw = setDT(waterUse_raw)
#------------------------------------------------------------------------------#
...
waterUse_ignore = waterUse_clean2

waterUse_yearly = waterUse_ignore |> 
  select(year, use_type, year_gallons) |> 
  group_by(year, use_type) |> 
  summarize(total_use = sum(year_gallons))

# waterUse_yearly = waterUse_yearly |> 
...
#                                    "Power", use_type)))
# 
# waterUse_yearly2 = waterUse_yearly |> 
#   mutate(use_type = ifelse(use_type %in% c("Power (Hydro-Elec)", 
#                                                   "Power (Fossil-Fuel)", 
#                                                   "Power (Geothermal)",
#                                                   "Geothermal"),
#                                   "Power", use_type))

ag_use2 = waterUse_yearly2 |> 
  filter(use_type == "Agricultural" & year > 1978) |> 
  mutate(total_use = total_use / 1000000)
