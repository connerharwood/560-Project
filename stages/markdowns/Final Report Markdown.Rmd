...
In the first set of models, we look at the association between water use by each sector and year-to-year GSL volume changes, while controlling for precipitation and population. Each water use by sector is log transformed since they are strongly right-skewed (see Appendix Figure 3) and contain outliers. We lag all independent variables by one year, since we expect a delayed effect of water use, precipitation, and population on lake volume. Further, we run the model using monthly water use and precipitation, and we impute values for monthly population by assuming that yearly population changes happen linearly from month to month. The results for the first set of models are shown below:
